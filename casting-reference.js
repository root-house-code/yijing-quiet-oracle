// casting-reference.js — authoritative, VERIFIED casting algorithms.
// Empirical frequencies (2M+ trials) match the theoretical distributions below.
// Hand this to Claude Code as the source of truth for src/lib/casting.js.
...
export function castReading(method, { authenticYarrow = true, rng = Math.random } = {}) {
  const fn =
    method === 'yarrow'
      ? (authenticYarrow ? castLineYarrow : castLineYarrowSimplified)
      : castLineCoins;
  return Array.from({ length: 6 }, () => fn(rng));
}

// Derive both hexagrams from line values.
...
// ---------------------------------------------------------------------------
// VERIFICATION HARNESS — run: `node casting-reference.js`
// ---------------------------------------------------------------------------
function verify(fn, n = 2_000_000) {
  const c = { 6: 0, 7: 0, 8: 0, 9: 0 };
  for (let i = 0; i < n; i++) c[fn()]++;
  return Object.fromEntries([6, 7, 8, 9].map(k => [k, (c[k] / n).toFixed(4)]));
}
